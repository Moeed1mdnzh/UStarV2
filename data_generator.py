...
max_val = 3 * max_val * ((len(list(range(1, 72, DATA_CONTROL)))*14)+(len(list(range(1, 151, 30)))*14))
                ...
                for cluster in raw_sample:
                    for img in cluster:
                        cv2.imwrite(os.sep.join(
                            ["dataset", "images", f"sample_{index}.jpg"]), img)
                        index += 1
                        pbar.update(index, ImageNumber=index)
                for cluster in raw_label:
                    for img in cluster:
                        cv2.imwrite(os.sep.join(
                            ["dataset", "labels", f"label_{index_2}.jpg"]), img)
                        index_2 += 1
